...
const genAI = new GoogleGenerativeAI(
	process.env.GEMINI_API_KEY || 'AIzaSyDa99ffhBLMlFEMVAj8jW8kKbYxxcMKkT4'
)
...
Your response:`
		...
		const validatedSkills = skills.map((skill) => {
			if (!skill.skillName || typeof skill.weight !== 'number') {
				throw new Error(
					'Invalid skill format: Missing skillName or weight'
				)
			}
		...
		throw new Error(
			`Failed to analyze project requirements: ${error.message}`
		)
	}
}
...
function matchSkillToDatabase(geminiSkillName, dbSkills) {
	...
	match = dbSkills.find((skill) =>
		skill.name.toLowerCase().includes(normalizedGeminiSkill)
	)
	if (match) return match

	// 3. Reverse - check if Gemini skill contains DB skill
	match = dbSkills.find((skill) =>
		normalizedGeminiSkill.includes(skill.name.toLowerCase())
	)
	if (match) return match
	...
	if (geminiVariation) {
		match = dbSkills.find(
			(skill) =>
				skill.name.toLowerCase() === geminiVariation ||
	...
	match = dbSkills.find((skill) => {
		...
		return (
			dbVariation === normalizedGeminiSkill ||
