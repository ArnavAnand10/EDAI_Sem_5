...
async function createSkill(req, res) {
  try {
    ...
    const skill = await prisma.skill.create({ data: { name, category: category || null } });
...
async function requestSkill(req, res) {
  try {
    ...
    if (!skillId || !level) return res.status(400).json({ error: 'skillId and level are required' });
...
async function updateEmployeeSkillStatus(req, res) {
  try {
    ...
    const employeeSkill = await prisma.employeeSkill.findUnique({ where: { id: parseInt(id) } });
    if (!employeeSkill) return res.status(404).json({ error: 'EmployeeSkill not found' });

    // Optional: Only allow admin of the employee to update
    const employee = await prisma.employee.findUnique({ where: { id: employeeSkill.employeeId } });
