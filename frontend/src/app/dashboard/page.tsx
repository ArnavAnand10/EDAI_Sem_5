          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {mySkills.length === 0 ? (
                  <p className="text-gray-500 text-center py-4">No skills requested yet</p>
                ) : (
                  mySkills.map((skillItem) => (
                    <div key={skillItem.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                      <div>
                        <p className="font-medium">{skillItem.skill.name}</p>
                        <p className="text-sm text-gray-600">{skillItem.level}</p>
                      </div>
                      <Badge className={getStatusColor(skillItem.status)}>
                        {skillItem.status}
                      </Badge>
                    </div>
                  ))
                ...
                {allSkills.map((skill) => (
                  <div key={skill.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                    ...
                    <div className="flex space-x-2">
                      <Button size="sm" onClick={() => requestSkill(skill.id, 'Beginner')}>
                        Request
                      </Button>
                    </div>
                  </div>
